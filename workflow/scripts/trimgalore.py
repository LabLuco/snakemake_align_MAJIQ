...
def execute_trim(rep,exp1,exp2,scriptdir):
    ...
    for exp in samples.keys():
        for namerep in samples[exp] :
            ...
            subprocess.run(('trim_galore --paired --fastqc --cores 4 --output_dir '+scriptdir+'/../../results/trim_galore/'+id+' '+fwd+' '+rev),stdout=subprocess.PIPE, stderr=subprocess.PIPE,universal_newlines=True,shell=True)
